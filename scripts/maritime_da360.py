...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Adapt DA360 to a downward-looking maritime fisheye video or image."
    )
    ...
    parser.add_argument("--model_path", required=True, type=str, help="DA360 checkpoint path.")
    parser.add_argument("--model_name", default="DA360_large", type=str, help="Label used in outputs.")
    parser.add_argument("--net", default=None, type=str, help="Override model architecture if needed.")
    parser.add_argument("--device", default=None, type=str, help="cuda, cpu, or leave unset for auto.")
    parser.add_argument("--erp_height", default=None, type=int, help="ERP height. Defaults to checkpoint value.")
    parser.add_argument("--erp_width", default=None, type=int, help="ERP width. Defaults to checkpoint value.")
    parser.add_argument(
        "--fisheye_fov_deg",
        default=200.0,
        type=float,
...
def fill_unseen_zenith(erp_rgb: np.ndarray, valid_mask: np.ndarray) -> np.ndarray:
    ...
    if cap_rows.size:
        ...
        blurred = cv2.GaussianBlur(filled, (0, 0), sigmaX=max(width / 80.0, 1.0), sigmaY=max(cap_height / 6.0, 1.0))
...
def load_model(args: argparse.Namespace, device: torch.device) -> Tuple[torch.nn.Module, int, int]:
    ...
    if args.erp_height is not None:
        checkpoint["height"] = args.erp_height
    if args.erp_width is not None:
        checkpoint["width"] = args.erp_width
    ...
    model.load_state_dict({k: v for k, v in checkpoint.items() if k in state_dict}, strict=False)
    model.to(device)
    model.eval()
    return model, int(checkpoint["height"]), int(checkpoint["width"])


def infer_depth(model: torch.nn.Module, erp_rgb_filled: np.ndarray, valid_mask: np.ndarray, device: torch.device) -> np.ndarray:
    inputs = normalize_rgb(erp_rgb_filled).to(device)
    with torch.no_grad():
        outputs = model(inputs)
    pred_disp = outputs["pred_disp"].detach().cpu().numpy()[0, 0]
    pred_depth = 1.0 / np.maximum(pred_disp, 1e-8)
    min_valid_depth = float(pred_depth[valid_mask].min()) if np.any(valid_mask) else float(pred_depth.min())
    pred_depth = pred_depth / max(min_valid_depth, 1e-8)
...
def process_frame(
    ...
    erp_depth = infer_depth(model, erp_filled, erp_valid_mask, device)
